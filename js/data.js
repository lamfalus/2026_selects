...
const PURPLE_WEAKNESSES = [
  ...
  { title: "Defensive shot quality allowed — team-wide pattern", body: "Lamfalusi's −0.208 net xG/GP (48 GP, 51% CORSI) is net-negative, but context matters: every D on his Jr. Sharks 13O team posted negative net xG/GP, and he was the best among them. Hannan (−0.28), Pan (−0.42), Suhara (−0.43), Amagata (−0.46), and Ivmark (−0.52) were all worse on similar samples. This reflects a team-level issue — opponents generated dangerous shot quality against the Sharks as a system — not a personal defensive liability unique to Lamfalusi." },
...
const PURPLE_GAME_PLAN = [
  ...
  { n: 7, title: "Don't over-index on Lamfalusi's net xG — it's a team effect", body: "His −0.208 net xG/GP looks like a liability but his 51% CORSI and team context tell a different story: every D on his Jr. Sharks squad posted negative net xG/GP, and Lamfalusi was the least negative of the group. The shot quality allowed reflects how that team played as a system, not a personal tendency to surrender dangerous chances. Treat him as a neutral defensive matchup rather than a primary target." }
];
...
const PURPLE_OVERVIEW_FINDINGS = {
  ...
  weaknesses: [
    ...
    "D shot quality allowed: team-wide pattern, not individual liability"
  ]
};

const PURPLE_POSS_FOOTNOTE = "* Lucas 78% FO on 27 draws — moderate volume, promising but not definitive. Cruz 46% on 214 draws and Yi 46% on 57 draws are the most reliable FO readings. Quire SC% 67% = 3 goals on ~4 scoring chances over 20 GP — very small base, use with caution. Lamfalusi SC% 4.5% reflects high scoring-chance volume (~67 SC over 48 GP, only 3 goals). Note: Lamfalusi's −0.208 net xG/GP should be read in team context — he was the best D on his Jr. Sharks squad by this metric; all six Sharks D posted negative net xG/GP on large samples, indicating a system-level effect rather than a personal defensive liability.";
